...
from utils.utils import ditch_detector, step_detector, data_preprocessing
from utils.hyperparameters import if_visual_main
    ...
    for count_beam, scan in enumerate(scan_generator()):
        ...
        ########## if angle == 0, that means 1 time is done ##########
            ...
            if count_scan >= 2: ##### use scan data to detect from 2nd scan 
                ...
                print()
                print('---------- Descend ----------')
                ditch_detector(scan_data, data)


                ##### Ascend / Step #####
                print()
                print('---------- Ascend ----------')
                _, _ = step_detector(scan_data, data)
